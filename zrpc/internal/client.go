...
func (c *client) dial(server string, opts ...ClientOption) error {
	...
	defer cancel()
	// 这里的server包含了grpc-go库中的一个etcd的schema， 在连接grpc的时候， 通过这个schema在
	// grpc-go库中可以找到go-zero resolver.go中提前注册到grpc-go中的这个resolver的map， map中包含了， resolver.go中从etcd中获取到的grpc的服务中的地址
	// grpc连接server的时候， 会将这些从etcd中拿到的地址建立和服务端的连接。
