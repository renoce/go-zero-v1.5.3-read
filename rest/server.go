...
type (
	...
	Server struct {
		ngin   *engine      // rest 框架引擎， 负责构建中间件调用链以及路由的组装
		router httpx.Router // 具体实现在patroute.go中， 负责路由的搜索树的构建， 构建一个handler以供golang stdlib server 链接使用，通过这个handler解析http数据， 找到最终应用路由函数处理
	}
)
...
func NewServer(c RestConf, opts ...RunOption) (*Server, error) {
	...
	// server配置， 通过RunOption选项， 将外部的传参给到RunOption的闭包函数， 通过闭包函数， 将参数给到Server中
...
// 保存应用层添加的一组路由， 以及这个路由群组的特有的鉴权， 超时，content-length等
func (s *Server) AddRoutes(rs []Route, opts ...RouteOption) {
	r := featuredRoutes{
		routes: rs,
	}
	// 初始化路由群组中的featuredRoutes中的timeout， maxBytes， signature等参数
	for _, opt := range opts {
		opt(&r)
	}
	// 将路由加入到featuredRoutes slice
	s.ngin.addRoutes(r)
}
